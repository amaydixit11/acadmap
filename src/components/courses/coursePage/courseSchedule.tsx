...
export function CourseSchedule({ course }: CourseScheduleProps) {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-4 sm:p-6">
      <h2 className="text-lg sm:text-xl font-bold text-gray-800 flex items-center mb-6">
        <Calendar className="mr-3 w-6 h-6 sm:w-7 sm:h-7 text-primary" /> 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
        {scheduleItems.map(({ label, icon: Icon, hours, color, bgColor }) => (
          <div 
            key={label} 
            className={cn(
              "group relative bg-white border border-gray-100 rounded-2xl p-5 sm:p-6",
          ...
          >
            ...
            <div className="max-md:flex-grow">
              <h3 className="text-base sm:text-lg font-semibold mb-2 text-gray-800 max-md:mb-0">
                {label}
              </h3>
              <div className="mt-auto w-full">
                <p className="text-2xl sm:text-3xl font-bold text-gray-900 flex items-baseline">
                  {hours}
