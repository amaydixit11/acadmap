...
export function getSlotOccurrenceNumber(
  day: string,
  time: string,
  slotLetter: string
): number {
      ...
      const checkSlot = (
        ...
        for (const parsedSlot of parsedSlots) {
          if (
            ...
            (parsedSlot.numbers.includes(occurrenceNumber) ||
              slotStr === slotLetter)
          ) {
            coursesInSlot.push({
...
export const parseCSV = (csvText: string): Promise<TimeTableParsedCourse[]> => {
  return new Promise((resolve, reject) => {
    Papa.parse(csvText, {
      header: true,
      complete: (results) => {
        const courses: TimeTableParsedCourse[] = results.data.map(
          (row: any) => {
            const [lectureSlot, lectureVenue]: [string, string] = row[
              "Slot Lecture\\Venue"
            ]?.split("\\") || ["NA", "NA"];
            const [tutorialSlot, tutorialVenue]: [string, string] = row[
              "Tutorial Slot\\Venue"
            ]?.split("\\") || ["NA", "NA"];
            const [labSlot, labVenue]: [string, string] = row[
              "Lab slot\\Venue"
            ]?.split("\\") || ["NA", "NA"];
